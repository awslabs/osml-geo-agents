...
def append_operation(
    dataset_georefs: List[Georeference], workspace: Workspace, function_name: str, output_format: str = "parquet"
) -> str:
    """
    Combine multiple geodataframes into a single result by appending them.

    :param dataset_georefs: List of Georeferences for the datasets to combine
    ...
    try:
        ...
        for georef in dataset_georefs:
            with LocalAssets(georef, workspace) as (item, local_assets):
                items.append(item)
        ...
        append_dataset_summary = (
            ...
            f"{len(dataset_georefs)} datasets: {', '.join(dataset_titles)}. "
        )

        # Write the derived dataset to the local workspace cache
        append_dataset_reference = Georeference.new_from_timestamp(asset_tag="combined", prefix=function_name)

        # Create a temporary directory for the append dataset
        temp_dir = Path(tempfile.gettempdir())
        append_dataset_path = temp_dir / append_dataset_reference.item_id / f"append-result.{output_format}"
        ...
        text_result = (
            f"The {len(dataset_georefs)} datasets have been combined into a single dataset. "
