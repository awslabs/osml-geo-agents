...
class TestFilterOperation(unittest.TestCase):
    ...
    @patch("aws.osml.geoagents.spatial.filter_operation.load_geo_data_frame")
    @patch("aws.osml.geoagents.spatial.filter_operation.STACReference")
    def test_filter_operation_query_expression(
        self, mock_stac_reference, mock_load_geo_data_frame, mock_create_derived_stac_item, mock_local_assets
    ):
        """Test successful filtering of features using a query expression."""
        # Set up mocks for LocalAssets context manager
        mock_item = Mock(spec=Item)
        mock_item.properties = {"title": "Test Dataset"}
        mock_local_asset_paths = {"asset1": Path("/tmp/asset1.parquet")}
        ...
        # Create mock GeoDataFrame with points and additional columns
        points = [
            shapely.Point(0.5, 0.5),
            shapely.Point(1.5, 1.5),
            shapely.Point(0.2, 0.2),
        ...
        # Configure load_geo_data_frame to return the mock GeoDataFrame
        ...
        # Call the operation function with a query expression
        query_expression = "population > 1000"
        result = filter_operation(
            function_name=self.function_name,
            workspace=self.mock_workspace,
            dataset_reference=self.dataset_reference,
        ...
        # Verify the result contains expected text
        self.assertIn("filtered", result)
        self.assertIn(query_expression, result)

        # Verify the mocks were called
        self.assertEqual(mock_local_assets.call_count, 1)
        self.assertEqual(mock_load_geo_data_frame.call_count, 1)

        # Verify load_geo_data_frame was called with correct parameters
        mock_load_geo_data_frame.assert_called_once_with(
            mock_local_asset_paths, self.mock_workspace, self.dataset_reference, mock_item, None
        )
    ...
    @patch("aws.osml.geoagents.spatial.filter_operation.load_geo_data_frame")
    @patch("aws.osml.geoagents.spatial.filter_operation.STACReference")
    def test_filter_operation_combined(
        self, mock_stac_reference, mock_load_geo_data_frame, mock_create_derived_stac_item, mock_local_assets
    ):
        """Test successful filtering of features using both spatial filter and query expression."""
        # Set up mocks for LocalAssets context manager
        mock_item = Mock(spec=Item)
        mock_item.properties = {"title": "Test Dataset"}
        mock_local_asset_paths = {"asset1": Path("/tmp/asset1.parquet")}
        ...
        # Call the operation function with both spatial filter and query expression
        query_expression = "population < 1000"
        result = filter_operation(
            function_name=self.function_name,
            workspace=self.mock_workspace,
